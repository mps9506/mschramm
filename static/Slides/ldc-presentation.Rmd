...
#Flow Duration Curve
```{r echo=FALSE, fig.align='center', fig.dim=c(8,3), message=FALSE, warning=FALSE, dpi=, paged.print=FALSE}
fdc
```
...
---
Now we can use dplyr to group the observations by flow category (the new fc column) and calculate teh geometric mean in each category
```{r}
df_summary <- df %>%
  group_by(fc) %>%
  summarise(AllowableLoad = Gmean(ldc, na.rm = FALSE),  ## Gmean function is from DescTools

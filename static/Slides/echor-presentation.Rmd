...
- If your query returns a large number of permits your dataframe will be empty. UPDATE: This is fixed!
...
## Create dataframe of permit numbers

```{r}
permits <- df %>%
  filter(CWPPermitTypeDesc == "NPDES Individual Permit") %>%
  select(SourceID)
```
...
DMR <- downloadDMRs(permits, SourceID, pBar = FALSE,
                    parameter_code = "51040",
                    start_date = "01/01/2010", 
                    end_date = "12/30/2017")

# DMR includes a list of dataframes

```

---

## Unnest the `dmr` column
...
- note that all the columns are returned as character type
...
ggplot(data = DMR, aes(x = as.Date(monitoring_period_end_date, "%m/%d/%Y"),
                       y = as.numeric(dmr_value_nmbr))) +
  geom_step(aes(color = statistical_base_short_desc), alpha = 0.5) +
  geom_point(aes(color = statistical_base_short_desc), alpha = 0.5) +
  facet_wrap(~SourceID) +
  scale_y_log10() +
  theme_light() +
  labs(x = "Date", y = "Value")
